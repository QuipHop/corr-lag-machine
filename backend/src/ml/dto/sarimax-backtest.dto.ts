...
class FeaturesCfgDto {
    @IsString() targetCode!: string;
    @IsOptional() @IsArray() @IsString({ each: true }) features?: string[];
    @IsOptional() lags?: Record<string, number>;
}
...
class AutoGridDto {
    @IsOptional() p?: [number, number];
    @IsOptional() d?: [number, number];
    @IsOptional() q?: [number, number];
    @IsOptional() P?: [number, number];
    @IsOptional() D?: [number, number];
    @IsOptional() Q?: [number, number];
    @IsOptional() @IsInt() s?: number;
    @IsOptional() @IsInt() max_models?: number;
}
class TrainCfgDto {
...
class BacktestCfgDto {
    @IsInt() horizon!: number;
    @IsInt() min_train!: number;
    @IsInt() step!: number;
    @IsBoolean() expanding!: boolean;
}

export class SarimaxBacktestDto {
    ...
    @ValidateNested() @Type(() => BacktestCfgDto)
    backtest!: BacktestCfgDto;
}
